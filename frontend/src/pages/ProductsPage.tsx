...
const ProductsPage = () => {
  ...
  const { addToCart, totalItems, cartItems, clearCartError } = useCart();
  ...
  const handleAddToCart = async (product: Product) => {
    ...
    try {
      ...
      showToastMessage(error instanceof Error ? error.message : 'Could not add to cart');
    }
  };
        ...
        <div className={`flex-1 min-w-0 ${cartOpen ? 'mr-80' : ''}`}>
          ...
          >
            {notification && (
              <div
                className={`${
                  notification.toLowerCase().includes('stock') ||
                  notification.toLowerCase().includes('could not')
                    ? 'bg-red-600 border-red-400'
                    : 'bg-green-500 border-green-400'
                } text-white px-4 py-2 rounded-lg shadow-lg border text-sm flex items-center gap-2`}
              >
                <span>
                  {notification.toLowerCase().includes('stock') ||
                  notification.toLowerCase().includes('could not')
                ...
                >
                  ...
                  <div className="p-6 space-y-4 flex-1 flex flex-col">
                    ...
                    <button
                      ...
                      className="w-full bg-primary text-white font-semibold py-3 rounded-lg hover:bg-primary-focus transition flex items-center justify-center gap-2 mt-auto disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-primary"
                    >
